...
def has_generator(floor):
    return _coconut_tail_call(any, map(lambda x: x[1] == 'g', floor))


@_coconut_tco
def all_chips_are_paired(floor):
    """The chips are safe because they are paired with their generators."""
    chips = _coconut.set((x[0] for x in floor if x[1] == 'm'))
    generators = _coconut.set((x[0] for x in floor if x[1] == 'g'))
    return _coconut_tail_call(chips.issubset, generators)


@_coconut_tco
def floor_is_safe(floor):
    """No chips will be fried."""
    if not (has_generator)(floor):
        return True
    return _coconut_tail_call((all_chips_are_paired), floor)
...
def get_possible_components_that_can_leave_the_floor(floor):
    ...
    return _coconut_tail_call(map, lambda s: frozenset(s), chain(combinations(floor, 1), combinations(floor, 2)))


def valid_floor_movements(state):
    """Get valid floors that the elevator can travel to."""
    return _coconut.set((state['elevator'] + 1, state['elevator'] - 1)) & _coconut.set((0, 1, 2, 3))


@_coconut_tco
def get_safe_new_state(state, new_elevator, items_to_move):
    """Return new state after items have moved to new floor if valid."""
    new_testing_facility = tuple(((existing_floor | items_to_move if i == new_elevator else existing_floor - items_to_move if i == state['elevator'] else existing_floor) for (i, existing_floor) in enumerate(state['testing_facility'])))

    both_floors_are_safe = floor_is_safe(new_testing_facility[state['elevator']]) and floor_is_safe(new_testing_facility[new_elevator])

    if both_floors_are_safe:
        return _coconut_tail_call(Map, {"elevator": new_elevator, "testing_facility": new_testing_facility})


@_coconut_tco
def safe_options_to_try(state):
    """Return the safe new states that can proceed a current state."""
    valid_next_floors = _coconut.set((state['elevator'] + 1, state['elevator'] - 1)) & _coconut.set((0, 1, 2, 3))
    floor_to_investigate = state['testing_facility'][state['elevator']]
    possible_components_that_can_leave_the_floor = get_possible_components_that_can_leave_the_floor(floor_to_investigate)
    results = _coconut.set((get_safe_new_state(state, new_elevator, items_to_move) for new_elevator, items_to_move in product(valid_next_floors, possible_components_that_can_leave_the_floor)))
    return _coconut_tail_call(filter, lambda x: x, results)
...
def get_move_count(move_count, current_move_states, visited_states, all_on_top_level):
    while True:
        ...
        all_on_top_level, current_move_states, visited_states = get_new_states_after_new_move(current_move_states, visited_states)

        try:
            _coconut_is_recursive = get_move_count is _coconut_recursive_func_9
        except _coconut.NameError:
            _coconut_is_recursive = False
        if _coconut_is_recursive:
            move_count, current_move_states, visited_states, all_on_top_level = move_count + 1, current_move_states, visited_states, all_on_top_level
            continue
        else:
            return _coconut_tail_call(get_move_count, move_count + 1, current_move_states, visited_states, all_on_top_level)



        return None
_coconut_recursive_func_9 = get_move_count
...
'''
    ...
    STATE = Map({'testing_facility': (frozenset([('hydrogen', 'm'), ('lithium', 'm'),]), frozenset([('hydrogen', 'g'),]), frozenset([('lithium', 'g'),]), frozenset([]),), 'elevator': 0})
