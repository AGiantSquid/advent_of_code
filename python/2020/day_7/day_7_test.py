...
from day_7 import count_parent_possibilities, count_children
...
def test_count_children():
    res = count_children(data)
    assert res == 32


def test_count_children_2():
    res = count_children(data_2)
    assert res == 126
...
def test_count_children_pd():
    res = count_children(PUZZLE_DATA)
    assert res == 158730
